    ...
    >
      <button
        ...
        className={`border h-${height} ${borderColor} ${disabled && "cursor-not-allowed "
            ...
            : "text-black bg-white"
          } rounded-lg truncate focus:ring-2 focus:ring-[#19525A8C] flex justify-between w-full h-full pl-[14px] pr-2 items-center ${bg && 'bg-[#19525A] text-[#fff]'} ${departmentError && 'border-red-500'} ${practitionerError && 'border-red-500'}`}
        type="button"
      >
        {selectedItemName}
        <BiChevronDown className={`text-2xl ml-1 ${bg ? 'text-[#fff]' : 'text-[#5B5B5B]'} `} />
      </button>
      {isOpen && (
        <div
          style={{ width: width }}
