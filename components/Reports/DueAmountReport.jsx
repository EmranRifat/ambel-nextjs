...
import DueAmountReportInfo from "./DueAmountReportInfo";
import FilterAllService from "../FilterAllService/Index";

const DueAmountTableHead = [
  {
    id: 1,
    name: "Date",
  },
...
export default function DueAmountReport() {
            ...
            <div className="flex w-full">
              <div className="w-full m-0 flex">
                ...
                <div className="flex gap-3 ml-3">
                  ...
                  <FilterAllService></FilterAllService>
                </div>
          ...
          <div className="flex gap-2">
            <table className="w-full">
              <thead className="border-b-2">
                ...
                {DueAmountTableHead.map((tableHead) => (
                  <th
                    key={tableHead.id}
              ...
              <tbody className="w-full">
                {DueAmountReportInfo.map((info) => (
                  <tr key={info.id} className="border-b-2">
                    ...
                    <td className="text-sm p-3 py-[14px] w-[220px] text-[#0089C9]">
                      {info.customername}
                    </td>
                    <td className="text-sm p-3 py-[14px] w-[240px] text-[#5b5b5b]">
                      <p className="text-sm">{info.serviceName}</p>
                      <p className="text-xs italic ">
                        By <span className="text-[#0089C9]">{info.doneby}</span>
                      </p>
                    </td>
                    <td className="text-sm p-3 text-center py-[14px]  w-[150px] text-[#5b5b5b]">
                      {info.paidAmount}
                    </td>
                    <td className="text-sm p-3 text-center py-[14px]  w-[150px] text-[#5b5b5b]">
                      {info.dueAmount}
